		...
		while(lineCtr < NUM_DATA_LINES )
			...
			// get line leader, check for failure
				...
				if( getLineTo( fileAccessPtr, MAX_STR_LEN, COLON,
					...
					// set end state messaging to line capture failure
						// function: copyString
			...
			if( dataLineCode != CFG_CORRUPT_PROMPT_ERR )
				...
				// get data value
					...
					// or CPU checuling names (strings)
					...
					if( valueInRange( dataLineCode, intData, doubleData, dataBuffer ) == True )
						...
						{
						// assign to data pointer depending on confid item
							// functionL getCpuSchedCode, getLogToCode
						switch( dataLineCode )
							{
					...
					// otherwise, ssume data value not in range
					else
						{
						// free temp struct memory
							// function: free
...
Postcondition: string parameter holds correct string associated with given constant
Exceptions: none
Notes: none
*/
void configCodeToString( int code, char *outString )
    { 
		// Define arry with eight items, and short (10) lengths
			...
			case CFG_VERSION_CODE:
				...
				case CFG_CPU_SCHED_CODE:
					...
					case CFG_QUANT_CYCLES_CODE:
						...
						// check for quantim cycles limits exceeded
...
Exceptions: mdefaults to fcfs code
Notes: none
*/
ConfigDataCodes getCpuSchedCode( char *codeStr )
    { 
		...
		// set default return to FCFS-NULL
...
Exceptions: mdefaults to monitor code
Notes: none
*/
ConfigDataCodes getLogToCode( char *logToStr )
    {
		...
		if( compareString(tempStr, "both") == STR_EQ )
			{
			// set return value to both coded
		...
		if( compareString(tempStr, "file") == STR_EQ )
			{
			// set return value to filter code
