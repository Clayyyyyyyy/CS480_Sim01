...
Algorithm: acquires the metadata items from file, stores them in linked list
...
Boolean getMetaData ( char *fileName, 
                        OpCodeType **opCodeDataHead, char *endStateMsg )
	{
		...
		*opCodeDataHead = NULL;
		...
		// (where complete op commands are found)
		...
		if( accessResult == LAST_OPCMD_FOUND_MSG )
			{
			// check for start and end op code counts as equal
			if( startCount == endCount )
				{
				// add the last node to thd linked list
					// function: addNode
				localHeadPtr = addNode( localHeadPtr, newNodePtr );
		...
		// check for any errors found (no not error)
...
int getOpCommand( FILE *filePtr, OpCodeType *inData )
    {
		...
		/*
		* The code cannot run correctly if I don't comment these lines
...
Algorithm: adds op command structure with data to linked list
...
OpCodeType *addNode( OpCodeType *localPtr, OpCodeType *newNode )
	{
		// check for local pointer assigned no null
		if( localPtr == NULL )
			{
		...
		// assign recursive function to currents next link
			// function: addNode
...
Precondition: input string is provided
Postcondition: in correct operation,
				reports if given string is in argument list,
				False if not
Exceptions: none
Notes: none
*/
